...
public class TributasLogger implements ILog{
	...
	/**
	 * Devuelve la cadena que se usará para identificar la carpeta en donde
	 * se escribirán los log. 
	 * Esta carpeta por el momento se crea en función de la fecha, como
	 * AAAA_MM_DD donde AAAA= Año actual, MM = número de mes actual, DD= número de día del mes
	 * De esa forma, habrá una copia de los log por día.
	 * @return Cadena AAAA_MM_DD que se usará como carpeta en donde escribir los log
	 */
	 ...
	 * @param idSesion
	 */
	public TributasLogger(NIVEL_LOG nivel,String dirRaizLog, String ficheroLog, String idSesion)
	{
		this.nivel= nivel;
		this.idSesion= idSesion;
		String dir=dirRaizLog;
		if (dir.endsWith("/")){
			dir= dir.substring(0, dir.length()-1);
		}
		this.fichero= dir+"/"+getCarpetaLog()+"/"+ficheroLog;
	}
	/**
	 ...
	 * @param ficheroLog
	 */
	public TributasLogger(NIVEL_LOG nivel, String dirRaizLog, String ficheroLog)
	{
		this(nivel, dirRaizLog, ficheroLog, null);
	}
	/**
	 * Constructor, permite indicar el nivel deseado de log, el fichero y el identificador de sesión
	 * que se incluirá en cada línea
	 ...
	 * @param idSesion
	 */
	public TributasLogger(String nivel,String dirRaizLog, String ficheroLog, String idSesion)
	{
		...
		//El problema de esto puede ser la sincronización,
		//dos procesos pueden intentar crear los directorios.
		//El segundo que lo intente "simultáneamente" no lo conseguiría porque el sistema operativo 
		//lo impediría, por eso no compruebo el resultado de mkdirs(), 
		//porque si no puede por existir, no me importa. Es que así no se necesita sincronización.
		
		File dirLog= new File(dir+"/"+getCarpetaLog());
		if (!dirLog.exists()){
			dirLog.mkdirs();
		} 
		this.fichero= dir+"/"+getCarpetaLog()+"/"+ficheroLog;
	 ...
	 * @param ficheroLog
	 */
	public TributasLogger(String nivel, String dirRaizLog, String ficheroLog)
	{
		this(nivel, dirRaizLog, ficheroLog, null);
	}
