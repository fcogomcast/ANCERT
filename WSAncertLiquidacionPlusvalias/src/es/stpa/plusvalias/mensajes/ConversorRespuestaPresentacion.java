 ...
 *
 */
public class ConversorRespuestaPresentacion {
	...
	private es.stpa.plusvalias.SujetoType getDatosSujeto(Persona sujeto){
		...
		if (sujeto.getDomicilio()!=null){
			...
			din.setBLOQUE(toOptional(sujeto.getDomicilio().getBloque()));
	 ...
	 * @return
	 */
    private PAGOLIQUIDACION.REPLY.TRANSMITENTES  getTransmitentes ( List<Transmitente> transmitentes){
		if (transmitentes==null) {
			return null;
		}
		PAGOLIQUIDACION.REPLY.TRANSMITENTES ret = new PAGOLIQUIDACION.REPLY.TRANSMITENTES();
    ...
    /**
     * Conversión de {@link Adquirente} a {@link PAGOLIQUIDACION.REPLY.ADQUIRENTES}
     * @param adquirentes
     * @return
     */
    private PAGOLIQUIDACION.REPLY.ADQUIRENTES  getAdquirentes ( List<Adquirente> adquirentes){
		if (adquirentes==null){
			return null;
		}
		PAGOLIQUIDACION.REPLY.ADQUIRENTES ret = new PAGOLIQUIDACION.REPLY.ADQUIRENTES();
    ...
    /**
     * Conversión de {@link es.stpa.plusvalias.domain.intercambio.Inmueble} a {@link PAGOLIQUIDACION.REPLY.INMUEBLE}
     * @param inmueble
     * @return
     */
    private PAGOLIQUIDACION.REPLY.INMUEBLE getInmueble(es.stpa.plusvalias.domain.intercambio.Inmueble inmueble){
    	if (inmueble==null){
    		return null;
    	}
    	PAGOLIQUIDACION.REPLY.INMUEBLE inm = new PAGOLIQUIDACION.REPLY.INMUEBLE();
    	...
    	if (inmueble.getDireccion()!=null){
    		...
    		d.setBLOQUE(inmueble.getDireccion().getBloque());
    ...
    /**
     * Conversión de {@link List} de {@link Persona} a {@link PAGOLIQUIDACION.REPLY.REPRESENTANTES}
     * @param representantes
     * @return
     */
    private PAGOLIQUIDACION.REPLY.REPRESENTANTES getRepresentantes(List<Persona> representantes){
    	if (representantes==null || representantes.size()==0){
    		return null;
    	}
    	PAGOLIQUIDACION.REPLY.REPRESENTANTES r = new PAGOLIQUIDACION.REPLY.REPRESENTANTES();
    ...
    /**
     * Conversión de {@link List} de {@link DocumentoType} a {@link PAGOLIQUIDACION.REPLY.DOCUMENTOS}
     * @param documentos
     * @return
     */
    private PAGOLIQUIDACION.REPLY.DOCUMENTOS getDocumentos(List<es.stpa.plusvalias.domain.intercambio.DocumentoType> documentos){
    	PAGOLIQUIDACION.REPLY.DOCUMENTOS docs= new PAGOLIQUIDACION.REPLY.DOCUMENTOS();
    	if (documentos==null){
    		return null;
    	}
    	for (es.stpa.plusvalias.domain.intercambio.DocumentoType d: documentos){
    		PAGOLIQUIDACION.REPLY.DOCUMENTOS.DOCUMENTO elem= new PAGOLIQUIDACION.REPLY.DOCUMENTOS.DOCUMENTO();
    		PAGOLIQUIDACION.REPLY.DOCUMENTOS.DOCUMENTO.IMPORTE imp = new PAGOLIQUIDACION.REPLY.DOCUMENTOS.DOCUMENTO.IMPORTE ();
    ...
    /**
     * Conversión de {@link ResultadoType} a {@link PAGOLIQUIDACION.REPLY.RESULTADO}
     * @param resultado
     * @return
     */
    private PAGOLIQUIDACION.REPLY.RESULTADO getResultado(ResultadoType resultado){
    	PAGOLIQUIDACION.REPLY.RESULTADO r = new PAGOLIQUIDACION.REPLY.RESULTADO();
    	if (resultado==null){
    		return null;
    	}
    	
    	if (resultado.getError()!=null){
    		PAGOLIQUIDACION.REPLY.RESULTADO.ERROR error = new PAGOLIQUIDACION.REPLY.RESULTADO.ERROR();
     ...
     * @return
     */
    public PAGOLIQUIDACION.REPLY from (RespuestaOperacionType reply){
    	PAGOLIQUIDACION.REPLY ret= new PAGOLIQUIDACION.REPLY();
